    ...
    const IntRange& local)
{
    ...
    kernelCount++; // Increase kernel count only after successful kernel addition and configurator creation
    return id;
}
...
void ExtendedTuner::printToScreenAll() const
{
    for (size_t id = 0; id < kernelCount; id++)
    {
        printToScreen(id);
...
void ExtendedTuner::printToFileAll(const std::string& filename) const
{
    for (size_t id = 0; id < kernelCount; id++)
    {
...
void ExtendedTuner::printResults(const size_t id, std::ostream& out) const 
{
    ...
    out << extDuration << best.extendedComputationDuration << extMs << std::endl;
...
void ExtendedTuner::storeTunerResult(const size_t id, const cltune::PublicTunerResult& result, const float extendedComputationDuration)
{
    ...
    extendedResult.extendedComputationDuration = extendedComputationDuration;
